...
export default function ResearchPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-linear-to-r from-purple-950/40 via-slate-900 to-slate-900 p-6 rounded-2xl border border-purple-500/20 shadow-xs">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form
          ...
          className="bg-white backdrop-blur-md border border-slate-200 rounded-2xl p-6 space-y-5 h-fit shadow-xs"
        >
          {/* Topic */}
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-sm text-slate-900 focus:border-purple-500 focus:outline-none transition resize-none leading-relaxed"
              required
            />
          </div>
          ...
          <div className="space-y-2 p-3.5 rounded-2xl bg-purple-950/25 border border-purple-500/30">
            ...
            <div className="flex items-center gap-2 pt-1">
              <input
                ...
                className="w-full accent-purple-500 cursor-pointer h-2 bg-slate-50 rounded-lg"
              />
                ...
                {[1, 3, 5, 10, 15].map((cnt) => (
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer border ${
                      versionsCount === cnt
                        ? "bg-purple-600 text-slate-900 border-purple-400"
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs text-slate-900 focus:border-purple-500 focus:outline-none transition resize-none leading-relaxed"
            />
          </div>

          {/* Target Pages */}
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 block">الحجم التقديري للبحث المطبوع:</label>
                ...
                <button
                  ...
                  className={`py-2 px-1 text-center rounded-xl text-xs font-bold transition cursor-pointer border ${
                    !isCustomPages && targetPages === pages
                      ? "bg-purple-600 text-slate-900 border-purple-500 shadow-md shadow-purple-600/20"
            ...
            {isCustomPages && (
              <div className="mt-2 flex items-center gap-2 p-2 rounded-xl bg-purple-950/20 border border-purple-500/30">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 focus:border-purple-400 focus:outline-none"
                />
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            <span className="text-xs font-bold text-purple-400 block border-b border-slate-200 pb-1.5">
              بيانات صفحة الغلاف الرسمية:
            </span>

            <div className="space-y-2 text-xs">
              <div>
                <label className="text-[11px] text-slate-500 block mb-1">اسم الطالب / الباحث:</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-200 rounded-xl pr-8 pl-3 py-1.5 text-xs text-slate-900 focus:border-purple-500 focus:outline-none"
                  />
                </div>
              </div>

              <div>
                <label className="text-[11px] text-slate-500 block mb-1">اسم الأستاذ / المشرف:</label>
                <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[11px] text-slate-500 block mb-1">الصف / الفرقة:</label>
                  <input
                    type="text"
                    value={gradeOrClass}
                    onChange={(e) => setGradeOrClass(e.target.value)}
                    placeholder="مثال: الصف الثالث الثانوي"
                    className="w-full bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs text-slate-900 focus:border-purple-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-slate-500 block mb-1">العام الدراسي:</label>
                  <input
                    type="text"
                    value={academicYear}
              ...
              <div>
                <label className="text-[11px] text-slate-500 block mb-1">المدرسة / الكلية والجامعة:</label>
                <div className="relative">
          ...
          <div className="space-y-2 pt-1 border-t border-slate-200">
            <button
              ...
              className="w-full flex items-center justify-between text-xs py-2 px-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 hover:border-purple-500/40 transition cursor-pointer"
            >
          ...
          <button
            ...
            className="w-full py-3.5 px-4 rounded-2xl bg-linear-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-slate-900 font-extrabold text-sm shadow-xs shadow-purple-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
          >
            {isLoading ? (
              <>
        ...
        {/* Results Panel */}
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 flex flex-col justify-between min-h-[550px] shadow-xs">
          {generatedText ? (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-1.5 py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 py-2 px-4 rounded-xl bg-linear-to-r from-purple-600 to-indigo-600 hover:opacity-90 text-slate-900 text-xs font-bold shadow-lg shadow-purple-600/30 transition disabled:opacity-50 cursor-pointer"
                  >
                  ...
                  {generatedVersions.length > 1 && (
                    <button
                      ...
                      className="flex items-center gap-2 py-2 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-purple-300 border border-purple-500/30 text-xs font-bold shadow-md transition disabled:opacity-50 cursor-pointer"
                    >
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 max-h-[600px] overflow-y-auto whitespace-pre-wrap text-slate-200 text-sm leading-relaxed font-sans select-text">
                {generatedText}
              </div>
            </div>
          ) : isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-lg mx-auto w-full">
              ...
              <div className="relative mb-6">
                <div className="w-20 h-20 rounded-2xl bg-purple-500/20 border border-purple-500/40 flex items-center justify-center text-purple-400 animate-pulse shadow-xs shadow-purple-600/20">
                  <Sparkles className="w-10 h-10 animate-spin text-purple-400" style={{ animationDuration: "3s" }} />
                </div>
                <div className="absolute -inset-2 bg-linear-to-r from-purple-500/30 to-indigo-500/30 rounded-2xl blur-xl -z-10" />
              </div>
              ...
              {/* Visual Progress Bar */}
              <div className="w-full bg-slate-50 rounded-full h-3 p-0.5 border border-slate-200 mb-6 overflow-hidden">
                <div
              ...
              {/* Step Checklist */}
              <div className="w-full bg-slate-900/40 rounded-2xl border border-slate-200 p-4 text-right space-y-2.5 text-xs">
                <div
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {sources.map((source, index) => (
            <div
              key={source.id}
              className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 hover:border-purple-500/40 hover:bg-slate-100/40 transition-all flex flex-col justify-between text-right space-y-2 group relative"
            >
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-200 group-hover:text-slate-900 leading-snug">
                  {source.name}
                </h4>
                <p className="text-[11px] text-slate-500 mt-1 line-clamp-2 leading-relaxed">{source.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {showAddSourceModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-purple-500/30 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-purple-500/20 text-purple-400 flex items-center justify-center font-bold">
                  <Plus className="w-4 h-4" />
                </div>
                <h3 className="text-sm font-bold text-slate-900">إضافة مصدر أو قاعدة بيانات بحثية</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleAddSource} className="space-y-3.5 text-right">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">اسم المصدر أو الدورية العلمية:</label>
                <input
                  type="text"
                  value={newSourceName}
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">التصنيف أو التخصص:</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">نبذة موجزة عن المصدر:</label>
                <textarea
                  rows={2}
                  value={newSourceDesc}
                  onChange={(e) => setNewSourceDesc(e.target.value)}
                  placeholder="مثال: مجلة علمية فصلية محكمة تنشر أحدث الأبحاث والدراسات في العلوم الإنسانية..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs text-slate-900 focus:border-purple-500 focus:outline-none resize-none"
                />
              </div>
